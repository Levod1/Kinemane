...
ADMIN_ID =6159567008
bot = telebot.TeleBot("6591271006:AAFoJqBUiakydYzdHFIgW4SuOVxh3wx_NB4",parse_mode='html')  
...
def add_new_kino_caption(msg):
    if msg.text:
        ...
        try:
            ...
            b = bot.send_video(msg.chat.id,file_id,caption=caption.replace("||","'").replace("%stat","0").replace("%code",str(code)),reply_markup=video_key(code)).message_id
...
def admin_menu():
    key =InlineKeyboardMarkup().add(
        ...
        InlineKeyboardButton(text="📊 Statisika",callback_data='static'),
        ...
        InlineKeyboardButton(text="➕ Kino",callback_data='add_new_kino'),
    ).add(
