...
def welcome(msg):
    cid = msg.chat.id
    id = str(msg.text).replace("/start","")
    try:
        id = int(id)
    except:
        id = "12345678"
    if join(cid):
        check = cursor.execute(f"SELECT * FROM video WHERE id={id}").fetchone()
        if check is None:
            bot.reply_to(msg,"<b>❌ Kino kodi noto'g'ri!</b>")
        else:
            try:
                down = check[3]+1
                ...
                cursor.execute(f"UPDATE video SET down = {down} WHERE id = {id}")
...
def send_media(message: Message):
    ...
    if text.isdigit() and join(cid):
        ...
        if check is None:
            ...
            try:
                down = check[3]+1
                bot.send_video(cid,check[1],caption=str(check[2]).replace('||',"'").replace("%stat",str(down)).replace("%code",str(text)),reply_markup=video_key(text))
